...
public static class PatchHelper
{
	...
	{
		string version = VersionHelper.PublishVersion;
		...
		if (version.Equals("0.6.8") || version.Equals("0.6.9") || version.Equals("0.7.0"))
		{
			...
			Dictionary<string, Source> sources = DnscryptProxyConfigurationManager.DnscryptProxyConfiguration.sources;
		...
		{
			//removed: daemonize
			string configFile = System.IO.Path.Combine(System.IO.Directory.GetCurrentDirectory(), Global.DnsCryptProxyFolder, Global.DnsCryptConfigurationFile);
			if (!System.IO.File.Exists(configFile))
				return false;
			TomlSettings settings = Nett.TomlSettings.Create(s => s.ConfigurePropertyMapping(m => m.UseTargetPropertySelector(standardSelectors => standardSelectors.IgnoreCase)));
