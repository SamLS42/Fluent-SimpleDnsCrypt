...
public class AboutViewModel : Screen
{
	...
	public void HyperlinkRequestNavigate(string type)
	{
		try
		{
			...
			if (type.Equals("code"))
			{
				if (SelectedLicense.LicenseCodeLink == null)
					return;
				if (SelectedLicense.LicenseCodeLink.LinkUri == null)
					return;
				Process.Start(SelectedLicense.LicenseCodeLink.LinkUri.ToString());
			}
			else if (type.Equals("regular"))
			{
				if (SelectedLicense.LicenseRegularLink == null)
					return;
				if (SelectedLicense.LicenseRegularLink.LinkUri == null)
					return;
